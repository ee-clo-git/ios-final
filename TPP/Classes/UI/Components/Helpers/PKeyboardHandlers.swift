...
extension PKeyboardObservable where Self : UIViewController {
    func onKeyboardAppear(handler:@escaping ((CGRect) -> Void)) {
        self.keyboardNotificationsObserver.addObserver(
    ...
    func onKeyboardDissappear(handler:@escaping () -> Void) {
        ...
        self.keyboardNotificationsObserver.addObserver(forName: notificationName, object: nil, queue: OperationQueue.main) { (notification) in
            handler()
        }
    }
}
